...
def video2image(videopath, imagepath, fps=0, start_time=0, last_time=0):
    if start_time == 0:
        if fps == 0:
            os.system('ffmpeg -v 0 -i "' + videopath + '" -f image2 ' + '-q:v -0 ' + imagepath)
        else:
            os.system('ffmpeg -v 0 -i "' + videopath + '" -r ' + str(fps) + ' -f image2 ' + '-q:v -0 ' + imagepath)
    else:
        if fps == 0:
            os.system(
                'ffmpeg -v 0 -ss ' + start_time + ' -t ' + last_time + ' -i "' + videopath + '" -f image2 ' + '-q:v -0 ' + imagepath)
        else:
            os.system('ffmpeg -v 0 -ss ' + start_time + ' -t ' + last_time + ' -i "' + videopath + '" -r ' + str(
                fps) + ' -f image2 ' + '-q:v -0 ' + imagepath)
...
def continuous_screenshot(videopath, savedir, fps):
    ...
    if fps == 0:
        os.system(
            'ffmpeg -v 0 -i "' + videopath + '" -f image2 ' + '-q:v -0 ' + savedir + '/' + videoname + '_%06d.jpg')
    else:
        os.system('ffmpeg -v 0 -i "' + videopath + '" -r ' + str(
            fps) + ' -f image2 ' + '-q:v -0 ' + savedir + '/' + videoname + '_%06d.jpg')
