...
from z_util import util, data
from z_util import image_processing as impro
from z_models import unet_model, BiSeNet_model
...
opt.parser.add_argument('--batchsize', type=int, default=16, help='')
...
opt.parser.add_argument('--dataset', type=str, default='./datasets/addmosaic', help='')
...
# dir_img = os.path.join(opt.dataset, 'train_B')
# dir_mask = os.path.join(opt.dataset, 'train_A')
...
for epoch in range(opt.startepoch, opt.maxepoch):
    ...
    for i in range(int(img_num * 0.8 / opt.batchsize)):
        img, mask = loadimage(imagepaths_train[i * opt.batchsize:(i + 1) * opt.batchsize],
                              maskpaths_train[i * opt.batchsize:(i + 1) * opt.batchsize], opt)
    ...
    with torch.no_grad():
        ...
        for i in range(int(img_num * 0.2 / opt.batchsize)):
            img, mask = loadimage(imagepaths_eval[i * opt.batchsize:(i + 1) * opt.batchsize],
                                  maskpaths_eval[i * opt.batchsize:(i + 1) * opt.batchsize], opt, test_flag=True)
            if opt.model == 'UNet':
                mask_pred = net(img)
            elif opt.model == 'BiSeNet':
...
'''
python3 z_train_addmosaic.py --gpu_id 0 --dataset ./datasets/addmosaic --savename face --loadsize 286 --finesize 256 --batchsize 16
